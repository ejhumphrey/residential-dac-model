...
\title{CarbonCloud - Feasibility Study}
\subtitle{A Distributed Residential Direct Air Capture Concept}

\author{%
  Rishu Agarwal\textsuperscript{1},
  Maarten Bosum\textsuperscript{2},
  Nathaniel Granor\textsuperscript{3},
  Eric J. Humphrey\textsuperscript{4},
  Jessica Maderos\textsuperscript{5}\\
  \textsuperscript{1}\texttt{agarwalrishu31@gmail.com},
  \textsuperscript{2}\texttt{maartenbosum@gmail.com},
  \textsuperscript{3}\texttt{nathaniel.granor@gmail.com},
  \textsuperscript{4}\texttt{humphrey.eric@gmail.com},
  \textsuperscript{5}\texttt{jessica.maderos@gmail.com}%
}
...
\begin{abstract}
Our considers a dual challenge: the urgent need for Gigaton-scale Carbon Dioxide Removal (CDR) and increasing economic pressure on households. This project investigated a solution to address both: a decentralized Direct Air Capture (DAC) network utilizing the existing HVAC infrastructure found in 75\% of American homes.

The core concept proposed a ``gig economy'' model for carbon: homeowners retrofit their forced-air systems with a smart filter containing a \ce{CO2}-selective sorbent. By leveraging airflow already paid for by the homeowner, we hypothesized this network could bypass the massive capital expenses and land-use requirements of centralized industrial DAC plants while providing passive income to families.

In anaylzing the concept, we determined the model requires a centralized plant for regeneration of \ce{CO2} gas from saturated sorbent, and thus also requires a logistics network to move sorbent between homes and the regeneration plant.

Our research decomposed this model into four distinct lanes of feasibility:
\begin{itemize}
    \item \textbf{Sorbent Viability:} The chemical capture mechanism is technically feasible. Research indicates Class IV Amine-based sorbents are currently the optimal choice for residential deployment, offering a capture capacity of $\approx 2.5$ mmol/g under standard indoor conditions. However, strictly residential constraints (e.g., the prohibition of toxic materials like \ce{NaOH}, a maximum comfortable lift weight of 10kg) limit the total capture volume per unit.
...
This white paper investigates the feasibility of a residential, distributed DAC network. Our research aims to answer the critical questions: Can we harness existing residential airflow for carbon capture? Is the lifecycle carbon negative? And finally, who pays for the captured carbon?
...
\begin{itemize}
    ...
    \item Section 4 models the logistics of a decentralized ``last-mile'' system, analyzing the emissions caused by distributing and reclaiming filters versus the carbon captured.
    ...
    \item Section 6 discusses the economics of this venture, analyzing markets for captured carbon dioxide, the revenue potential of selling food and beverage grade \ce{CO2} versus carbon credits and other potential commodities, and considering the costs of operation.
...
The concentration of greenhouse gases (GHGs) in the atmosphere drives climate change. The most widespread GHG is carbon dioxide. According to the Intergovernmental Panel on Climate Change, the average global temperature will stop rising when we stop increasing the concentration of GHGs in the atmosphere, i.e. when we reach "net-zero" emissions. Net-zero can be achieved by a combination of eliminating human-caused GHG emissions (for example, by replacing fossil fuels with renewable energy sources) and offsetting any remaining emissions with new carbon removals. 
...
\begin{enumerate}
    ...
    \item \textbf{Liquid Solvent:} This method moves air through a chemical solution such as potassium hydroxide (\ce{KOH}, also known as caustic potash) in an "air contactor" (like a cooling tower) where CO2 is absorbed into the solution oftentimes as a carbonate salt. 
\end{enumerate}

\textbf{Residential DAC} would be an innovative hybrid approach. It adopts the core solid sorbent technology typical of large-scale DAC but decentralizes the capture units. This distribution is specifically designed to capitalize on the distinct atmospheric and environmental conditions found within the built environment.
The residential setting offers a unique thermodynamic advantage:   
\begin{itemize}
    \item \textbf{\ce{CO2} Enrichment:} Human respiration elevates indoor \ce{CO2}  levels significantly above ambient. In an occupied bedroom or living room, levels frequently reach approximately around 800 ppm \cite{yuan2023indoor}.
...
\end{itemize}
The critical component of the Residential DAC system is the sorbent material. Unlike industrial settings where liquid solvents like caustic potash (\ce{KOH}) are acceptable, the residential context demands materials that are non-toxic, solid-state, and capable of operating at ambient temperatures and varying humidity levels.
...
We would note that given the higher concentrations of \ce{CO2} in indoor settings, it is possible that this sorbent capacity might be higher in practice.
...
For our purposes here, we linearly scale this value as a rough estimate (24, 16, and 8 stops in the urban, suburban, and rural conditions respectively).
...
Extrapolating this flux over one year ($8,760$ hours) for the total count of U.S. households ($132.2$ million) \cite{fred2024households}, we estimate the total addressable residential \ce{CO2} potential in Table \ref{tab:co2_estimates}.
...
However, on some level, the sorbent or other capture substrate will always represent a dead-weight tax on logistics. We argue, the sustainable long-term path is probably in-home regeneration capable of ``charging'' concentrated cannisters that can have small modular footprints, like a SodaStream or whipped cream charger. 
...
\begin{itemize}
    ...
    \item \textbf{Calcination:} Involves decomposition under high temperaturs (up to 950$^\circ$C) of solid calcium carbonate (\ce{CaCO3}) into \ce{CaO} and \ce{CO2}. Has reached pilot and commerical scale, for example with the companies Carbon Engineering, 1PointFive, and Heirloom.
...
There is a direct relationship between the financial cost to regenerate a sorbent and the energy input required. To analyze the net carbon negativity of the regeneration process, the metric of regeneration efficiency in kilowatt hours (kWh) per metric ton (mTonne) is examined.
...
While the electrical energy requirement to power the \textit{adsorption} process has been widely documented by various organizations, our in-home DAC system would theoretically have a 0 kWh/mTonne contribution due to the utilization of an already-present HVAC blower fan. On the other hand, the available literature on electrical energy requirements for regeneration is thin, with values heavily dependent on the scale of regeneration facility (a large central facility has the best efficiency). Regardless, \textit{thermal} energy dominates the costs in our application. Therefore, this analysis focuses on thermal energy costs alone.

According to published analyses, thermal energy consumption for chemisorbent based systems have been evaluated between 1170 and 2000 kWh/mTonne\cite{Leonzio_Fennell_Shah_2022}. 
With grid electricity rates in the US in late 2025 ranging between \$0.07 and \$0.42 per kWh depending on location, time of use, and residential vs commercial rates\cite{gridrates_2025}, we can calculate a best-case regeneration cost of \textbf{\$81.9/mTonne}. 
...
Even if the homeowner collected the full revenue each month (with no revenue used to cover costs or generate profit), it is hard to imagine this amount would suffice to incentivize a homeowner to sign up, let alone to lug an 11kg filter to the street and back each cycle.
...
\begin{itemize}
    ...
    \item Additional purification of the regenerated \ce{CO2}. Would most likely be required for food and beverage grade \ce{CO2}. 
\end{itemize}

Unfortunately, we needn't examine these costs deeply to determine that the costs are not borne by the minimal unit revenue calculated above. A very brief analysis of the logistics cost per replacement trip alone guarantees unprofitability in the U.S. market:
...
\begin{itemize}
    ...
    \item Assume a driver is paid \$20 per hour (it is likely higher), then each delivery/swap costs between \$0.83 and \$2.50. 
\end{itemize}

So the delivery driver cost alone costs 1--2x the total potential revenue of the \ce{CO2} captured by a cartridge. It is therefore clear that the proposed business model operates at a loss.
...
This creates a critical gap in the business model. To induce behavioral change and consistent participation, we estimate that household incentives would need to approach \$1,000 annually. Currently, the operational expense (OpEx) of the logistics network exceeds the value of the carbon recovered, making the unit economics more unfavorable. This structural deficit makes it difficult to subsidize the network upfront with Venture Capital funding. The key takeaway is that there is simply insufficient market demand for the molecule itself to support an ``AirBnB for Carbon'' narrative.
...
The economic failure of the distributed model is a function of the low market value of captured carbon. If the market for the output product expands, the entire logistics chain becomes viable. We see two distinct avenues for this work:
...
\begin{thebibliography}{99}
    \bibitem{ieefa2022} Robertson, B., and M. Mousavian,``Carbon Capture to Serve Enhanced Oil Recovery: Overpromise and Underperformance,'' \textit{Institute for Energy Economics and Financial Analysis (IEEFA)}, March 2022. Available: \url{https://ieefa.org/wp-content/uploads/2022/02/Carbon-Capture-to-Serve-Enhanced-Oil-Recovery-Overpromise-and-Underperformance_March-2022.pdf}
    \bibitem{eip2024adm} Gibbons, Brendan, ``In Illinois, a massive taxpayer-funded carbon capture project fails to capture about 90 percent of plant\'s emissions'' \textit{Oil \& Gas Watch}, 2024. Available: \url{https://news.oilandgaswatch.org/post/in-illinois-a-massive-taxpayer-funded-carbon-capture-project-fails-to-capture-about-90-percent-of-plants-emissions}
    \bibitem{seabound2025} Seabound, ``Seabound Launches World-First Onboard Marine Carbon Capture Project With Hartmann, InterMaritime, And Heidelberg Materials'' \textit{Carbon Capture Magazine}, July 2025. [Online]. Available: \url{https://carboncapturemagazine.com/articles/seabound-launches-world-first-onboard-marine-carbon-capture-project-with-hartmann-intermaritime-and-heidelberg-materials}
    \bibitem{soletairwebsite} ``Regenerative Building Pilot at Europaplads'' \textit{Soletair company website}. [Online]. Available: \url{https://www.soletairpower.fi/soletair-power-bicc-onsite-unit-aarhus-denmark/}
    \bibitem{carbonreformsite} ``The Carbon Reduction System (CRS)'' \textit{Carbon Reform website}. [Online]. Available: \url{https://www.carbonreform.com/carbon-reduction-system}
    \bibitem{cleantechnica2025ovi} Evans, Larry, ``Atalanta Climate Ovi: Indoor Carbon Capture at the Intersection of Health \& Climate Change'', \textit{CleanTechnica} October 2025. [Online]. Available: \url{https://cleantechnica.com/2025/10/22/atalanta-climate-ovi-indoor-carbon-capture-at-the-intersection-of-health-climate-change/}
    \bibitem{an2023} An, Keju, et al. “A Comprehensive Review on Regeneration Strategies for Direct Air Capture.” Journal of CO2 Utilization, vol. 76, 2023.
    \bibitem{house2011} House KZ, et al. Economic and energetic analysis of capturing CO2 from ambient air. Proc Natl Acad Sci U S A. 2011.
    \bibitem{li2024} Li, Sheng, and Zhong Zhang. “Prospects for Direct Air Capture.” The Innovation Energy, vol. 1, 2024.
    \bibitem{hoeger2021} Hoeger, Christopher, et al. “Cryogenic Carbon Capture Technoeconomic Analysis.” SSRN Electronic Journal, 2021.
    \bibitem{wang2012} Wang, Tao, et al. “Moisture-swing Sorption for Carbon Dioxide Capture From Ambient Air.” Physical Chemistry Chemical Physics, 2012.
    \bibitem{iea} IEA, Direct Air Capture. \url{https://www.iea.org/energy-system/carbon-capture-utilisation-and-storage/direct-air-capture}
    \bibitem{sabatino2022} Sabatino, Francesco, et al. “Modeling... Bipolar Membrane Electrodialysis.” Industrial \& Engineering Chemistry Research, 2022.
    \bibitem{keith2018process} D.~W. Keith, G.~Holmes, D.~St.~Angelo, and K.~Heidel, ``A process for capturing \ce{CO2} from the atmosphere,'' \emph{Joule}, vol. 2, no. 8, pp. 1573--1594, 2018.
    \bibitem{panda2023evaluation} Panda, D., Kulkarni, V. \& Singh, S. Evaluation of amine-based solid adsorbents for direct air capture: a critical review. {\em Reaction Chemistry \& Engineering}. \textbf{8}, 10-40 (2023)
    \bibitem{sanz2016direct}Sanz-Pérez, E., Murdock, C., Didas, S. \& Jones, C. Direct capture of \ce{CO2} from ambient air. {\em Chemical Reviews}. \textbf{116}, 11840-11876 (2016)
    \bibitem{shekhah2014made}Shekhah, O., Belmabkhout, Y., Chen, Z., Guillerm, V., Cairns, A., Adil, K. \& Eddaoudi, M. Made-to-order metal-organic frameworks for trace carbon dioxide removal and air capture. {\em Nature Communications}. \textbf{5}, 4228 (2014)
    \bibitem{redfern2019mechanical}Redfern, L. \& Farha, O. Mechanical properties of metal–organic frameworks. {\em Chemical Science}. \textbf{10}, 10666-10679 (2019)
    \bibitem{wang2025investigation}Wang, Y., Kim, J., Marreiros, J., Rangnekar, N., Yuan, Y., Johnson, J., McCool, B., Realff, M. \& Lively, R. Investigation of Moisture Swing Adsorbents for Direct Air Capture by Dynamic Breakthrough Studies. {\em ACS Sustainable Chemistry \& Engineering}. \textbf{13}, 6554-6564 (2025)

    \bibitem{yuan2023indoor}Yuan, J., Song, X., Yang, X., Yang, C., Wang, Y., Deng, G., Wang, Z. \& Gao, J. Indoor carbon dioxide capture technologies: a review. {\em Environmental Chemistry Letters}. \textbf{21}, 2559-2581 (2023)
    
   
    % Distribution
    \bibitem{noaa2024trends} NOAA Global Monitoring Laboratory, ``Trends in Atmospheric Carbon Dioxide,'' 2024. [Online]. Available: \url{https://gml.noaa.gov/ccgg/trends/weekly.html}.
    \bibitem{fred2024households} U.S. Census Bureau and Federal Reserve Bank of St. Louis, ``Total Households [TTLHH],'' 2024. [Online]. Available: \url{https://fred.stlouisfed.org/series/TTLHH}.
    \bibitem{eia2020recs} U.S. Energy Information Administration, ``Residential Energy Consumption Survey (RECS), Table HC10.9,'' U.S. Department of Energy, 2020. [Online]. Available: \url{https://www.eia.gov/consumption/residential/data/2020/hc/pdf/HC\%2010.9.pdf}.
    \bibitem{satish2012indoor} U. Satish et al., ``Is \ce{CO2} an Indoor Pollutant? Direct Effects of Low-to-Moderate \ce{CO2} Concentrations on Human Decision-Making Performance,'' \textit{Environmental Health Perspectives}, vol. 120, no. 12, pp. 1671--1677, 2012. Available: \url{https://pmc.ncbi.nlm.nih.gov/articles/PMC3548304/}.
    \bibitem{epa2024factors} U.S. Environmental Protection Agency, ``Exposure Factors Handbook: Chapter 19 (Building Characteristics),'' 2024. [Online]. Available: \url{https://www.epa.gov/system/files/documents/2025-01/efh-chapter19_508.pdf}.
    \bibitem{toolbox2024density} The Engineering ToolBox, ``Carbon Dioxide - Density and Specific Weight,'' 2024. [Online]. Available: \url{https://www.engineeringtoolbox.com/carbon-dioxide-density-specific-weight-temperature-pressure-d_2018.html}.
    \bibitem{eia2020hvac} U.S. Energy Information Administration, ``Residential Energy Consumption Survey (RECS), Table HC7.1 (Air Conditioning) and HC6.1 (Space Heating),'' U.S. Department of Energy, 2020. [Online]. Available: \url{https://www.eia.gov/consumption/residential/data/2020/hc/}.

    \bibitem{AN2023102587} An, Keju, et al. “A Comprehensive Review on Regeneration Strategies for Direct Air Capture.” Journal of CO2 Utilization, vol. 76, Sept. 2023, p. 102587. [Online]
    Available: \url{https://www.sciencedirect.com/science/article/pii/S2212982023001981?via%3Dihub}
    \bibitem{Leonzio_Fennell_Shah_2022} Leonzio Grazia, Fennell Paul S., \& Shah Nilay. (2022). Modelling and Analysis of Direct Air Capture Systems in Different Locations. Chemical Engineering Transactions, 96, 1–6. [Online]
    Available: \url{https://www.cetjournal.it/cet/22/96/001.pdf}
    \bibitem{gridrates_2025} ElectricChoice.com. “Electricity Rates by State – December 2025.” Electric Choice, 2 Dec. 2025, [Online] Available: \url{www.electricchoice.com/electricity-prices-by-state.}
    \bibitem{olivon25} Admin, Olivon. “The U.S. Food Grade CO2 Industry: An Essential Component of Food and Beverage Processing.” Olivon Advisors, 6 June 2025[Online]
    Available: \url{https://www.olivonadvisors.com/the-u-s-food-grade-co2-industry-an-essential-component-of-food-and-beverage-processing/}
   \bibitem{sudarshan25} Mining, and Mining. “How Much Does Calcium Carbonate Cost - Sudarshan Group.” Sudarshan Group -, 27 Sept. 2025, sudarshangroup.com/the-true-cost-of-industrial-materials-factors-trends-and-insights. [Online]
    Available: \url{https://sudarshangroup.com/the-true-cost-of-industrial-materials-factors-trends-and-insights/}
    \bibitem{lammert2010ups} M. Lammert, ``United Parcel Service Evaluates Hybrid Electric Delivery Vans,'' National Renewable Energy Laboratory (NREL), Golden, CO, Tech. Rep. NREL/TP-5400-47327, Apr. 2010. [Online]. Available: \url{https://www.nrel.gov/docs/fy10osti/47327.pdf}
    \bibitem{wef2020lastmile} World Economic Forum, ``The Future of the Last-Mile Ecosystem,'' World Economic Forum Reports, p. 11, Jan. 2020. [Online]. Available: \url{https://www3.weforum.org/docs/WEF_Future_of_the_last_mile_ecosystem.pdf}
    \bibitem{waters1994niosh} T. R. Waters, V. Putz-Anderson, and A. Garg, ``Applications Manual for the Revised NIOSH Lifting Equation,'' National Institute for Occupational Safety and Health (NIOSH), Cincinnati, OH, DHHS (NIOSH) Pub. No. 94-110, 1994.
    \bibitem{fasihi2019techno} M.~Fasihi, O.~Efimova, and C.~Breyer, ``Techno-economic assessment of \ce{CO2} direct air capture plants,'' \emph{Journal of Cleaner Production}, vol. 224, pp. 957--980, 2019.
    \bibitem{fccc2024mt45} Freightliner Custom Chassis Corp, ``MT45 Walk-In Van Chassis: Technical Specifications,'' Daimler Truck North America, Portland, OR, Prod. Sheet, 2024. [Online]. Available: \url{https://www.freightliner.com/chassis/mt-chassis/}
    \bibitem{rivian2025fleet} Rivian Automotive, ``Rivian Commercial Van: Fleet Specifications (Model Year 2025),'' Irvine, CA, Tech. Spec. Sheet, Feb. 2025. [Online]. Available: \url{https://stories.rivian.com/rivian-commercial-van-fleet}
